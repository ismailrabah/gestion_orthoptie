            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md">
                ...
                <div v-if="showModal && currentModel">
                    <jig-modal
                        ...
                        <template #title>Show Fichier #{{currentModel.id}}</template>
                        <show-fichiers-form :model="currentModel"></show-fichiers-form>
                        <template #footer>
                ...
                <div v-if="addModal">
                    <jig-modal
                        ...
                        <template #title>Nouveau Fichier</template>
                        <create-fichiers-form :patient="patient" @success="addSuccess" @error="addError"/>
                    </jig-modal>
                </div>
            </div>
        </div>
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            You are not authorized to view a list of Fichiers
        </div>
    </jig-layout>
</template>
...
<script>
    ...
    import CreateFichiersForm from "./CreateForm.vue";
    ...
    export default defineComponent({
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.fichiers.dt'));
        ...
        methods: {
            ...
            showModel(model) {
                axios.get(route('api.fichiers.show',model)).then(res => {
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.fichiers.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.fichiers.update`,model.id),{
                    enabled: enabled
                }).then(res => {
