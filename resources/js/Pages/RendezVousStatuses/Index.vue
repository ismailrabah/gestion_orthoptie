<template>
    <jig-layout>
        <template #header>
            <div class="flex flex-wrap items-center justify-between w-full px-4">
                ...
                <div class="flex gap-x-2">
                    <inertia-button v-if="can.create" :href="route('admin.rendez-vous-statuses.create')" classes="bg-green-100 hover:bg-green-200 text-primary"><i class="fas fa-plus"></i> New
                        Rendez Vous Status</inertia-button>
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md">
                ...
                <div v-if="showModal && currentModel">
                    <jig-modal
                        ...
                        <template #title>Show Rendez Vous Status #{{currentModel.id}}</template>
                        <show-rendez-vous-statuses-form :model="currentModel"></show-rendez-vous-statuses-form>
                        <template #footer>
        ...
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            You are not authorized to view a list of Rendez Vous Status
        </div>
    </jig-layout>
</template>
...
<script>
    ...
    import ShowRendezVousStatusesForm from "@/Pages/RendezVousStatuses/ShowForm.vue";
    ...
    export default defineComponent({
        ...
        data() {
            return {
                tableId: 'rendez-vous-statuses-dt',
                tableParams: {},
                datatable: null,
                confirmDelete: false,
                currentModel: null,
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.rendez-vous-statuses.dt'));
        ...
        methods: {
            showModel(model) {
                axios.get(route('api.rendez-vous-statuses.show',model)).then(res => {
                    this.currentModel = res.data.payload;
                    this.showModal = true;
                })
                // this.$inertia.visit(this.route('admin.rendez-vous-statuses.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.rendez-vous-statuses.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.rendez-vous-statuses.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.rendez-vous-statuses.update`,model.id),{
                    enabled: enabled
                }).then(res => {
