    ...
    public static function dt($query) {
        return DataTables::of($query)
            ...
            ->editColumn('actions', function (ConsultationTach $model) {
                $actions = '';
                if (\Auth::user()->can('view',$model)) $actions .= '<button class="bg-indigo-600 hover:bg-indigo-500 p-2 px-3 focus:ring-0 focus:outline-none text-white action-button" title="Imprimer la facture" data-action="print-invoice" data-tag="button" data-id="'.$model->id.'"><i class="fas fa-print"></i></button>';
