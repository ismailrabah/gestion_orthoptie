<template>
    <jig-layout>
        <template #header>
            <div class="flex flex-wrap items-center justify-between w-full px-4">
                ...
                <div class="flex gap-x-2">
                    <inertia-button v-if="can.create" :href="route('admin.taches.create')" classes="bg-green-100 hover:bg-green-200 text-primary"><i class="fas fa-plus"></i> Nouveau Tach</inertia-button>
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md">
                ...
                <div v-if="showModal && currentModel">
                    <jig-modal
                        ...
                        <template #title>Show Tach #{{currentModel.id}}</template>
                        <show-taches-form :model="currentModel"></show-taches-form>
                        <template #footer>
        ...
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            You are not authorized to view a list of Taches
        </div>
    </jig-layout>
</template>
...
<script>
    ...
    import ShowTachesForm from "@/Pages/Taches/ShowForm.vue";
    ...
    export default defineComponent({
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.taches.dt'));
        ...
        methods: {
            showModel(model) {
                axios.get(route('api.taches.show',model)).then(res => {
                    this.currentModel = res.data.payload;
                    this.showModal = true;
                })
                // this.$inertia.visit(this.route('admin.taches.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.taches.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.taches.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.taches.update`,model.id),{
                    enabled: enabled
                }).then(res => {
