<template>
    <jig-layout>
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md">
                ...
                <div v-if="showModal && currentModel">
                    <jig-modal
                        ...
                        <template #title>Show Salles d'examen #{{currentModel.id}}</template>
                        <show-salles-d-examens-form :model="currentModel"></show-salles-d-examens-form>
                        <template #footer>
        ...
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            You are not authorized to view a list of Salles D Examens
        </div>
    </jig-layout>
</template>
...
<script>
    ...
    import ShowSallesDExamensForm from "@/Pages/SallesDExamens/ShowForm.vue";
    ...
    export default defineComponent({
        ...
        computed: {
            ajaxUrl() {
                const url = new URL(this.route('api.salles-d-examens.dt'));
        ...
        methods: {
            showModel(model) {
                axios.get(route('api.salles-d-examens.show',model)).then(res => {
                    this.currentModel = res.data.payload;
                    this.showModal = true;
                })
                // this.$inertia.visit(this.route('admin.salles-d-examens.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.salles-d-examens.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.salles-d-examens.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.salles-d-examens.update`,model.id),{
                    enabled: enabled
                }).then(res => {
