<template>
    <jig-layout>
        <template #header>
            <div class="flex flex-wrap items-center justify-between w-full px-4">
                ...
                <div class="flex gap-x-2">
                    <inertia-button v-if="can.create" :href="route('admin.consultation-prestations.create')" classes="bg-green-100 hover:bg-green-200 text-primary"><i class="fas fa-plus"></i> New
                        Consultation Prestation</inertia-button>
            ...
            <div class="z-10 flex-auto bg-white md:rounded-md md:shadow-md">
                ...
                <div v-if="showModal && currentModel">
                    <jig-modal
                        ...
                        <template #title>Show Consultation Prestation #{{currentModel.id}}</template>
                        <show-consultation-prestations-form :model="currentModel"></show-consultation-prestations-form>
                        <template #footer>
        ...
        <div v-else class="p-4 font-bold text-red-500 bg-red-100 rounded-md shadow-md ">
            You are not authorized to view a list of Consultation Prestations
        </div>
    </jig-layout>
</template>
...
<script>
    ...
    export default defineComponent({
        ...
        data() {
            return {
                tableId: 'consultation-prestations-dt',
                tableParams: {},
                datatable: null,
                confirmDelete: false,
                currentModel: null,
        ...
        methods: {
            showModel(model) {
                ...
                // this.$inertia.visit(this.route('admin.consultation-prestations.show',model.id));
            },
            editModel(model) {
                this.$inertia.visit(this.route('admin.consultation-prestations.edit',model.id));
            },
            ...
            async deleteModel() {
                ...
                if (this.currentModel) {
                    this.$inertia.delete(route('admin.consultation-prestations.destroy', vm.currentModel),{
                        onFinish: res => {
            ...
            async toggleActivation(enabled,model) {
                const vm = this;
                console.log(enabled);
                axios.put(route(`api.consultation-prestations.update`,model.id),{
                    enabled: enabled
                }).then(res => {
